...
public class BusTest {
    ...
    public static void testValidCreationBus() {
        ...
        try {
            ...
            System.out.println("  Создан: " + String.valueOf(bus));
        } catch (Exception e) {
    ...
    public static void testPlateNumber() {
        ...
        String[] invalidPlateNumbers = new String[]{"123ABC48", "ABC12348", "А12ВС48", "А1234ВС48", "", null};

        for(String plateNumber : invalidPlateNumbers) {
            try {
                Bus bus = (new Bus.Builder()).setNumber("А123АА48").setModel("ПАЗ").build();
                System.out.println("✓ Тест пройден ");
            } catch (IllegalArgumentException e) {
                System.out.println("✗ Ошибка! \n Гос.номер: " + (plateNumber == null ? "null" : plateNumber) + " не прошел проверку.\n Исправьте:" + e.getMessage());
            }
        }
    ...
    public static void testInvalidModel() {
        ...
        try {
            (new Bus.Builder()).setNumber("А123АА48").setModel("МАЗ").setMileage(100).build();
            System.out.println("✓ Тест пройден");
        } catch (IllegalArgumentException e) {
    ...
    public static void testNegativeMileage() {
        ...
        try {
            Bus bus = (new Bus.Builder()).setNumber("А123АА48").setModel("Камаз 02").setMileage(100).build();
            System.out.println("✓ Тест пройден ");
        } catch (IllegalArgumentException e) {
            System.out.println("✗ Ошибка! \n Исправьте:" + e.getMessage());
        }
    ...
    public static void testEmptyFields() {
        ...
        try {
            Bus bus = (new Bus.Builder()).setNumber("").setModel("МАЗ").build();
            System.out.println("✓ Тест пройден");
        } catch (IllegalArgumentException e) {
            System.out.println("✗ Ошибка! \n Исправьте: " + e.getMessage());
        }
        ...
        try {
            Bus bus = (new Bus.Builder()).setNumber("А123АА48").setModel("").build();
